  ...
  const recordAudio = async () => {
    try {
      ...
      mediaRecorder.current.ondataavailable = (blob_event: BlobEvent) => {
        if (blob_event.data.size > 0) chunks.current.push(blob_event.data);
      };
      ...
      mediaRecorder.current.onstop = async () => {
        ...
        const audio_file = new File(chunks.current, "audio_record.weba", {
          type: "audio/webm",
        });
        const result = await send_message({
          content: "",
  ...
  return !user ? (
    ...
    >
      ...
      >
        ...
        <div
          className="card w-100 h-100 d-flex flex-column gap-3 p-4 rounded-0"
          style={{
            overflowY: "auto",
          }}
        >
        ...
        <Modal show={true}>
          ...
          >
            ...
            >
              ...
              <small className="mw-100">{showMessageInfoPopup?.content}</small>
            </div>
            ...
            <div>
              ...
              <small>
                {DATETIME_FORMATTER.format(showMessageInfoPopup?.creation_date)}
              </small>
            </div>
            {showMessageInfoPopup.seen_date && (
              <div>
                ...
                <small>
                  {DATETIME_FORMATTER.format(showMessageInfoPopup?.seen_date)}
                </small>
              </div>
            )}
          </Modal.Body>
